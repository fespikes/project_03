...
import { ColorSchema } from './common/color-schema';
import { LegendConfig } from './common/legend';
...
export class DonutChart implements ChartBase {
  ...
  drawLegend(data, idx, ledendWrapper?) {
    ...
    const legend = d3.select(ledendWrapper).append('svg')
      .attr('class', 'legend')
      .attr('width', legendStyle.width)
      .attr('height', (legendStyle.rectHeight + 4) * data.columns.length )
      // .style('position', 'absolute')
      // .style('left', config.style.left + idx * (config.style.left + legendStyle.width + 2 * config.style.maxRadius))
  ...
  drawTire() {
    ...
    donutWrapper.style = 'height: ' + (style.maxRadius * 2 * me.shadowSize - 5) + 'px; display: flex; justify-content: space-evenly; '
        + '; position: relative; left: -' + style.maxRadius + 'px';
    ...
    labelWrapper.style = 'height:50px; display: flex; justify-content: space-evenly; ' +
        '; position: relative; left: -' + (style.maxRadius + 5) + 'px; top: -' + (style.maxRadius + 10) + 'px';
    const legendWrapper: any = document.createElement('div');
    legendWrapper.style = 'height:100px; display: flex; justify-content: space-evenly; ' +
        '; position: relative; left:' + style.maxRadius + 'px; top: -' + (style.maxRadius + 100) + 'px';
    config.donutChartHolder.appendChild(donutWrapper);
    config.donutChartHolder.appendChild(labelWrapper);
    config.donutChartHolder.appendChild(legendWrapper);
    ...
    this.svg = d3.select(donutWrapper).selectAll('.pie')
      ...
      .each(function(d: any, idx) {
        ...
        me.drawLegend(d, idx, legendWrapper);
        ...
        path.selectAll('.arc')
          ...
          .enter().append('path')
          .attr('class', 'arc')
          .attr('d', me.arc.outerRadius(r).innerRadius(r * style.inner))
